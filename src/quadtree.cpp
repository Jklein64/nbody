...
void Quadtree::createLeaves(Node &node) {
    ...
    node.northEast =
        new Node(&node, node.xCenter + 0.25 * node.width,
                 node.yCenter + 0.25 * node.height, 0.5 * node.width, 0.5 * node.height);
    node.northWest =
        new Node(&node, node.xCenter - 0.25 * node.width,
                 node.yCenter + 0.25 * node.height, 0.5 * node.width, 0.5 * node.height);
    node.southEast =
...
bool Quadtree::checkIfBodyIsLost(Body &body) {
    ...
    if (fabs(body.xPos) > this->root.xCenter + this->root.width / 2.0 ||
        fabs(body.yPos) > this->root.yCenter + this->root.height / 2.0) {
        return true;
    } else {
        return false;
    }
