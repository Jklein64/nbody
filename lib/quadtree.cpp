...
void QuadTree::Build(const std::vector<glm::vec2>& positions,
    ...
    for (size_t i = 0; i < positions.size(); i++) {
        ...
        cell.center_of_mass = (cell.center_of_mass * cell.total_mass + 
                              positions[i] * masses[i]) / new_total_mass;
        ...
        while (parent_key != 0) {
            ...
            parent.center_of_mass = (parent.center_of_mass * (parent.total_mass - masses[i]) +
                                   positions[i] * masses[i]) / parent.total_mass;
...
glm::vec2 QuadTree::CalcAccel(size_t i, const glm::vec2& pos, float mass, float theta) const {
...
glm::vec2 QuadTree::CalcForceFromCell(const Cell* cell, const glm::vec2& pos, float theta_squared) const {
